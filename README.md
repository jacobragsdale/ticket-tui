...
browser. Title, Priority, Tags, and Add comment have no key of their own; they
are reached through `e`, or by name in the command palette as `Edit title`,
`Edit priority`, `Edit tags`, and `Add comment`. Assignee is reached the same
way, and also directly with `a`.
...
tags, leaves a comment, and exports the selection as JSON or CSV. Press `i` for database path, row count, freshness, and the last

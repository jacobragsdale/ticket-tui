...
Values in one field are ORed; different fields are ANDed. `search.pending` means
the fuzzy result worker has not yet published its latest matches, so the counts
below are a moment behind the query above.

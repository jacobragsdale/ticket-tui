...
use pickers::{
    render_agent_picker, render_assignee_picker, render_delete_confirm, render_edit_menu,
    render_form, render_link_picker, render_node_picker, render_parent_picker,
    render_priority_picker, render_prompt, render_state_picker, render_tag_picker,
    render_type_picker,
};
use table::{
...
fn render_pass(frame: &mut Frame<'_>, screen: &mut WorkItemsScreen, shell: &mut Shell, area: Rect) {
    ...
    match screen.mode {
        ...
        WorkItemMode::AgentPicker => render_agent_picker(frame, screen, shell),

...
pub(super) fn render_state_picker(
    frame: &mut Frame<'_>,
    screen: &mut WorkItemsScreen,
    shell: &mut Shell,
) {
    let options = screen.state_picker.options.clone();
    let current = screen.state_picker.current.clone();
...
pub(super) fn render_assignee_picker(
    frame: &mut Frame<'_>,
    screen: &mut WorkItemsScreen,
    shell: &mut Shell,
) {
    let candidates = screen.assignee_matches();
    let current = screen.assignee_picker.current.clone();
    ...
    let rows: Vec<Line> = candidates
        ...
        .collect();
    let width = overlay_width(shell.overlay_anchor, &rows, 52, frame.area());
    let area = overlay_area(frame.area(), shell.overlay_anchor, width, height);
    let title = format!(
        " Assignee \u{b7} {} ",
    ...
    let (text, cursor) = (
        ...
        screen.assignee_picker.query.cursor(),
    );
    render_query_field(
        frame,
        shell,
...
pub(super) fn render_parent_picker(
    ...
    let candidates = screen.parent_matches();
    let current = screen.parent_picker.current.clone();
    let height = u16::try_from(candidates.len().saturating_add(3))
        .unwrap_or(u16::MAX)
        .clamp(5, 18);
    ...
    let (text, cursor) = (
        ...
        screen.parent_picker.query.cursor(),
    );
    render_query_field(
        frame,
        shell,
...
/// `current`.
pub(super) fn render_node_picker(
    frame: &mut Frame<'_>,
    screen: &mut WorkItemsScreen,
    shell: &mut Shell,
) {
    let rows_data = screen.node_matches();
    let current = screen.node_picker.current.clone();
    let kind = screen.node_picker.kind;
    ...
    let rows: Vec<Line> = rows_data
        ...
        .collect();
    let width = overlay_width(shell.overlay_anchor, &rows, 56, frame.area());
    let area = overlay_area(frame.area(), shell.overlay_anchor, width, height);
    let title = format!(
        " {} \u{b7} {} ",
        kind.label(),
    ...
    let (text, cursor) = (
        ...
        screen.node_picker.query.cursor(),
    );
    render_query_field(
        frame,
        shell,
...
pub(super) fn render_prompt(
    ...
    let Some((field, text, cursor, id)) = screen.prompt.as_ref().map(|prompt| {
        (
            prompt.field,
...
pub(super) fn render_type_picker(
    frame: &mut Frame<'_>,
    screen: &mut WorkItemsScreen,
    shell: &mut Shell,
) {
    let options = screen.type_picker.options.clone();
    let current = screen.type_picker.current.clone();
    let height = u16::try_from(options.len().saturating_add(2))
        .unwrap_or(u16::MAX)
        .clamp(3, 16);
...
/// every form in the screen is drawn by this one function.
pub(super) fn render_form(frame: &mut Frame<'_>, screen: &mut WorkItemsScreen, shell: &mut Shell) {
    let Some((title, fields, selected)) = screen.form.as_ref().map(|form| {
        (
            form.title.clone(),
            form.fields.clone(),
    ...
    screen
        .scroll_state_mut(ScrollSurface::Form)
        .set_viewport(viewport, fields.len());
    let scroll = screen.scroll_state(ScrollSurface::Form).offset;
...
pub(super) fn render_delete_confirm(
    ...
    let Some(confirm) = screen.delete_confirm.clone() else {
        return;
    };
    let mut text = vec![
        Line::styled(
    ...
    capture_selectable(frame, shell, SelectableSurface::Overlay, chunks[0], false);
